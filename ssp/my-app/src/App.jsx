...
function App() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await axios.post('https://agri-jv1x.onrender.com/upload', formData);
  ...
  const fetchUploadedFiles = async () => {
    try {
      const res = await axios.get('https://agri-jv1x.onrender.com/files');
      const filesWithBlobs = await Promise.all(
        res.data.map(async (file) => {
          const response = await axios.get(`https://agri-jv1x.onrender.com/file/${file._id}`, {
